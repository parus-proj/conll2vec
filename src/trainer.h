...
//  }
  ...
  // усредненный вектор записывается по идексу, соответствующему первому элементу списка
...
//    // DBG-end
  ...
  inline void attract_vecs_s(float* vector1Ptr, float* vector2Ptr, size_t to_end, const ExtVocabExample& data)
  {
    ...
    f = sigmoid(f);
    if (f == 1.0) return; // уже очень похожи
    float g = 1.0 - f; // величина различия
    if ( g < data.e_dist_lim ) return; // требуем, чтобы стягиваемые вектора хоть немного, но различались, т.к. слова-то всё ж разные
    // здесь g>0
  ...
  inline void attract_vecs_e(float* vector1Ptr, float* vector2Ptr, size_t to_end, const ExtVocabExample& data, float* neu1e)
  {
    ...
    if ( e_dist < data.e_dist_lim) return; // требуем, чтобы стягиваемые вектора хоть немного, но различались, т.к. слова-то всё ж разные
    std::transform(neu1e, neu1e+to_end, neu1e, [alpha=alpha](float a) -> float {return a * alpha;});
...
private:
  ...
  void decrease_subsampling()
  {
    ...
    std::dynamic_pointer_cast<LearningExampleProviderLemmas>(lep)->update_subsampling_rates(0.5 /*, 0.95, 0.95*/); // выполняем первым, т.к. InitUnigramTable зависит от уже вычисленных sample_probability в словарях
    if (table_dep)
      free(table_dep);
    if ( dep_ctx_vocabulary )
      InitUnigramTable(table_dep, dep_ctx_vocabulary);
